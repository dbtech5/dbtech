...
$(document).ready(function () {
    ...
    $('#Basin_group').prop('checked', true)
    $("#Basin_group").trigger("change")
    $('#Basin').prop('checked', true)
    $("#Basin").trigger("change")
});

var lng_responsive = 15.0500
var view = new ol.View({
    minZoom: 2,
    maxZoom: 20,
    center: ol.proj.fromLonLat([103, lng_responsive]),
    zoom: 8.8,
    //extent: ol.proj.transformExtent([101.2, 14, 105.6, 16.5],'EPSG:4326', 'EPSG:3857')
    extent: ol.proj.transformExtent([100.2, 10, 105.6, 16.5],'EPSG:4326', 'EPSG:3857')
})
...
setTimeout(function(){
    if(window.innerWidth < 1000 && window.innerHeight < 500){
        ...
        }else if(window.innerWidth < 1800 && window.innerHeight < 900){
            map.getView().animate({
                center: ol.proj.fromLonLat([103, 15.0500]),
                duration: 0,
                zoom: 7.9
    ...
    var data_page = urlParams.get('layers');
    data_page = data_page.replace("[","").replace("]","").split(",")
    console.log(data_page)
    if(data_page!=null){
        data_page.forEach(data=>{
            console.log(data)
...
window.addEventListener('orientationchange',function(){
    ...
    /*
    document.querySelectorAll('[data-toggle="tooltip"]')[0].click()
    setTimeout(function(){
        console.log(latlng_hist)
    },1000)
    */
})
...
map.on('moveend', function(e) {
    ...
    if (minZoom <= view.getZoom()) {
        if(document.getElementById('SubDistrict').checked){
            removeLayer(layers['SubDistrict_Location'])
...
function zoom_SubDistrict(action){
    if(action){
        if ($('#SubDistrict').checked) {
            try{
                removeLayer(layers[$('#SubDistrict').name])
                removeLayer(layers['SubDistrict_Location'])
            }catch(err){

            }
            ilayer = $('#SubDistrict').name;
...
button.addEventListener('click', function () {
    ...
    */
    /*
    map.getView().animate({
        center: ol.proj.fromLonLat([103, 13.0500]),
        duration: 2500,
        zoom: 8.8
    });
    */
