...
// no label
function geojson_vt(opacity,ilayer, ilayerfile, ilayerobject, ifillcolor, istrokecolor, istrokewidth, ilinedash, izIndex, maxres) {
    if(opacity <= 1 && opacity >=0){
        
    }else{
        maxres = izIndex
        izIndex = ilinedash
        ilinedash = istrokewidth
        istrokewidth = istrokecolor
        istrokecolor = ifillcolor
        ifillcolor = ilayerobject
        ilayerobject = ilayerfile
        ilayerfile = ilayer
        ilayer = opacity
    }
    ...
    fetch(url).then(function (response) {
        ...
        if (ilayer == 'Basin') {
            ...
            layers[ilayer].setStyle(function (feature, resolution) {
                return new ol.style.Style({
                    fill: new ol.style.Fill({ color: ifillcolor }),
                    stroke: new ol.style.Stroke({
                        color: istrokecolor,
                        width: istrokewidth,
...
// parameter 10 ตัว
function topojson_label(ilayer, ilayerfile, ifont, itextfillcolor, itextstrokecolor, itextstrokewidth, ifillcolor, istrokecolor, istrokewidth, ilinedash, ilabel, maxres) {
...
// parameter 4 ตัว
function point_label(ilayer, ilayerfile, iconfile, markerName, iconscale) {
...
$('#Basin_group').change(function () {
    ...
    if (this.checked) {
        ...
        // ไม่ต้องแล้ว ใช้ class ทำทีเดียว
        // $('#Basin22').prop('checked', false)
        // $("#Basin22").trigger("change")
        // $('#Basin').prop('checked', false)
        // $("#Basin").trigger("change")
        // $('#SubBasin').prop('checked', false)
        // $("#SubBasin").trigger("change")
    }
});
...
$('#Admin_group').change(function () {
    ...
    if (this.checked) {
        ...
        // ไม่ต้องแล้ว ใช้ class ทำทีเดียว
        // const layergroup = ['Province', 'District', 'SubDistrict', 'Local_Admin', 'Village'];
        // layergroup.forEach(RemoveLayerGroup);

        // function RemoveLayerGroup(index) {
        //     map.removeLayer(layers[index]);
        // }
        // removefeatureInfo('Village')
    }
});
...
$('#Trans_group').change(function () {
    ...
    if (this.checked) {
        ...
        // ไม่ต้องแล้ว ใช้ class ทำทีเดียว                
        // const layergroup = ['Road', 'Railway', 'Trans_Station'];
        // layergroup.forEach(RemoveLayerGroup);

        // function RemoveLayerGroup(index) {
        //     map.removeLayer(layers[index]);
        // }
        // removefeatureInfo(['Trans_Station'])
    }
});

// ป่าไม้
$('#Forest_group').change(function () {
    var info = $(this).parent().parent().find('.filter_info')
    if (this.checked) {
        $(info).show()
    } else {
        $(info).hide()
        $('.forest_checkbox').prop('checked', false)
        $('.forest_checkbox').trigger("change")

        // ไม่ต้องแล้ว ใช้ class ทำทีเดียว
        // const layergroup = ['NPRK', 'WLDS', 'NHTA', 'NRF', 'PARK'];
        // layergroup.forEach(RemoveLayerGroup);
...
// Flood GISTDA
$('#FloodGISTDA_group').change(function () {
    var info = $(this).parent().parent().find('.filter_info')
    if (this.checked) {
        $(info).show()
    } else {
        $(info).hide()
        $('.floodgistda_checkbox').prop('checked', false)
        $('.floodgistda_checkbox').trigger("change")

        // remove layer group
        // var layergroup = [];
        // for (var i = 2548; i <= 2562; i++) {
        //     layergroup.push('Flood_GISTDA' + i);
        // }
        // layergroup.forEach(RemoveLayerGroup);
        // function RemoveLayerGroup(index) {
        //     map.removeLayer(layers[index]);
...
// Flood Tr
$('#Flood_Tr_group').change(function () {
    var info = $(this).parent().parent().find('.filter_info')
    if (this.checked) {
        $(info).show()
    } else {
        $(info).hide()
        $('.flood_tr_checkbox').prop('checked', false)
        $('.flood_tr_checkbox').trigger("change")

        // remove layer group
        // const layergroup = ['Flood_Tr2', 'Flood_Tr5', 'Flood_Tr10', 'Flood_Tr25', 'Flood_Tr50', 'Flood_Tr100'];
        // layergroup.forEach(RemoveLayerGroup);
